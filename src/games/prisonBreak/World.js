...
export function setupDefaultLocations() {
  const jailCell = {
    ...
    name: "My Cell",
    description:
      "It is a small room only big enough for you to lie down in with high stone walls.",
    type: "location",
    paths: [],
    x: 1,
    y: 0,
  ...
  const medicalWard = {
    ...
    name: "Medical Ward",
    description:
      "The medical room of the jail and there should be some healing supplies in here.",
    paths: [],
    type: "location",
    x: 0,
    y: 3,
...
export function createItems(locations) {
  ...
  const bandage = {
    ...
    name: "Potion",
    description:
      "A small potion of healing. Will restore all HP(Health Points) when used.",
    type: "consumable",
  ...
  const key = {
    ...
    name: "Entrance Key",
    description:
      'A small iron key that has the tag "Entrance Hall" attached to it.',
    type: "key",
