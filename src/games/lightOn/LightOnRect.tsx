...
export default class LightOnRect extends CanvasRect {
  ...
  public onClick(elements: CanvasElement[]) {
    if (this.active) {
      ...
      const above = lightOnRects.find(
        (rect) => rect.gridx === this.gridx && rect.gridy === this.gridy - 1
      );
      const below = lightOnRects.find(
        (rect) => rect.gridx === this.gridx && rect.gridy === this.gridy + 1
      );
      const left = lightOnRects.find(
        (rect) => rect.gridx === this.gridx - 1 && rect.gridy === this.gridy
      );
      const right = lightOnRects.find(
        (rect) => rect.gridx === this.gridx + 1 && rect.gridy === this.gridy
      );
